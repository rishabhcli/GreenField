...
| Toolchain: clean build + lint | **DONE** | `pnpm exec tsc -b tsconfig.build.json` green on this pass; `pnpm test` **729 passed / 13 skipped** (2026-08-15). A stale `.tsbuildinfo` can still false-pass — CI starts from `pnpm clean && pnpm build`. |
...
| Operator console (`apps/site/console.html`) | **PARTIAL** | Structural tests in `apps/site/test/console.test.ts` (22 passed): `$('id')` resolution, registered API paths, phase rail, provider squares, token stays out of `localStorage`, API base baked to `https://foundry-api-8ih0.onrender.com` (`YELLOFIELD_API_BASE` / `data-api` / no `location.origin` fallback; `?api=` persist only if allowlisted). Live reads now hit `foundry-api` (see below). Write paths still unproven in a browser session. |
| Landing honesty (`apps/site/index.html`) | **DONE** | Hero no longer uses `aria-label="Verified status"` or a frozen test/probe total. Integration chips are an illustrative snapshot dated 2026-08-15. Loop copy is 15 phases matching `LOOP_PHASE_ORDER`. RFQ mock says awaiting quotes. Landing tests pin NOT COMPLETE, Illustrative, and evidence-kind labels — not a frozen integer. |
| Verifier sole writer | **DONE** | `verification.probe` stays in `QUEUE_NAMES` / schemas / policies but is off `SCHEDULED_JOBS`. The worker handler returns `{ skipped: true, reason: 'verifier is the sole writer' }`. `qa.run_contract_tests` still probes but does not persist `integration_verifications` rows. |
| Operating-loop site/build wiring | **DONE** (unit) | `loop-assess-build.test.ts`: `spec_drafted` does not complete build; `drive('build')` dispatches `commerce_manager`; preview `site.deploy` is enqueued when the site is generatable; `research.cluster` uses `${cycleId}:discover:cluster`; `forcePhase` runs under the tick lock. `createSpec` writes `active_site_id` and copies the configured Render service id. `marketing.decide` / `resolveUnitContributionMinor` return blocked when contribution is unknown — no 0 placeholder. Not live loop evidence. |
| Dodo digital MoR (local) | **DONE** (unit) | `assertPaymentRoute` throws for physical goods; digital checkout/webhook/refund state machine with Standard Webhooks fixtures; GET /products HTTP 401 is `succeeded=false`; refund ledger keyed `refund:${orderId}:${amount}` and stored under `re_*`, not the payment id. Live probe still **HTTP 401** — not `live_verified`. Splices for Stripe-owned collect/checkout/refunds: `PATCH.md`. |
